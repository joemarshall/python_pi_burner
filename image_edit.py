...
def _add_setting(key, value, config_txt):
    ...
    for line in config_txt.splitlines():
        ...
        if comment_pos >= 0:
            ...
            if (
                len(line_no_comment.strip()) == 0
                and line[comment_pos + 1 :].strip() == commented_line_match
            ):
...
def add_dynamic_files(drive_path):
    ...
    git_date_result = subprocess.run(
        ["git", "show", "-s", "--format=%ci"],
        capture_output=True,
        cwd=Path(".") / "contents" / "home" / "pi" / "grove-startup-scripts",
        text=True,
    )
...
def create_init_files(options):
    if options.labimage == True:
        ...
        shutil.copyfile(
            "setup_dss_mac_address.sh", "installscripts/setup_dss_mac_address.sh"
        )
    else:
...
"""
            % (options.uniname, unipw, options.wifiname, options.wifipw),
            newline="\n",
        )
